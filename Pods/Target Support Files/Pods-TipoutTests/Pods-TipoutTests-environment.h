
// To check if a library is compiled with CocoaPods you
// can use the `COCOAPODS` macro definition which is
// defined in the xcconfigs so it is available in
// headers also when they are imported in the client
// project.


// Fox
#define COCOAPODS_POD_AVAILABLE_Fox
#define COCOAPODS_VERSION_MAJOR_Fox 1
#define COCOAPODS_VERSION_MINOR_Fox 0
#define COCOAPODS_VERSION_PATCH_Fox 1

// Nimble
#define COCOAPODS_POD_AVAILABLE_Nimble
#define COCOAPODS_VERSION_MAJOR_Nimble 1
#define COCOAPODS_VERSION_MINOR_Nimble 0
#define COCOAPODS_VERSION_PATCH_Nimble 0

// NimbleFox
#define COCOAPODS_POD_AVAILABLE_NimbleFox
#define COCOAPODS_VERSION_MAJOR_NimbleFox 1
#define COCOAPODS_VERSION_MINOR_NimbleFox 0
#define COCOAPODS_VERSION_PATCH_NimbleFox 0

// Quick
#define COCOAPODS_POD_AVAILABLE_Quick
#define COCOAPODS_VERSION_MAJOR_Quick 0
#define COCOAPODS_VERSION_MINOR_Quick 3
#define COCOAPODS_VERSION_PATCH_Quick 1

